library(pheatmap)
heatmaps_plot = function(meta_file, pvalues_file, show_rownames = T, show_colnames = T,
  ...
  if (any(count1)>0)
  {
    ...
    pheatmap(log(count_matrix+1), show_rownames = show_rownames, show_colnames = show_colnames, scale=scale, cluster_cols = cluster_cols,
             border_color=border_color, cluster_rows = cluster_rows, fontsize_row = fontsize_row, fontsize_col = fontsize_col, breaksList,

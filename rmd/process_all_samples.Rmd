...
ReadSTAR <- function(data.dir, proj = "SeuratObject", feats = 500, counts = 1000, 
  ...
  seurat = CreateSeuratObject(ReadMtx(mtx = mtx, cells = cells, features = features), 
         project = proj, min.features = feats)
  seurat = subset(seurat, subset=nCount_RNA >= counts)
...
gw19_2 = lapply(c(1:length(samples)), function(i){
  ReadSTAR(paste0(dirs,dir[[i]],"/"), proj = samples[[i]], sample = "GW19_2")
})
...
merged = merge(seurat, c(seurat2,seurat3,seurat4,seurat5,seurat6,seurat7,seurat8,
                         seurat9,seurat10,seurat11,pcw))
rm(seurat);rm(seurat2);rm(seurat3);rm(seurat4);rm(seurat5);rm(seurat6);rm(seurat7);rm(seurat8);rm(seurat9);rm(seurat10);rm(seurat11);rm(pcw);gc()

merged = merge(merged,c(cs19,cs20,cs20_2))
rm(cs19);rm(cs20);rm(cs20_2);gc()

merged2 = merge(cs22,c(cs22_3,gw14,gw14_2,gw16_v3))
rm(cs22);rm(cs22_3);rm(gw14);rm(gw14_2);rm(gw16_v3);gc()

merged2 = merge(merged2,c(gw17,gw18,gw18_2,gw18_layer))
rm(gw17);rm(gw18);rm(gw18_2);rm(gw18_layer);gc()

merged2 = merge(merged2,c(gw19,gw19_2,gw19_v3))
rm(gw19);rm(gw19_2);rm(gw19_v3);gc()
...
X_center <- rowMeans(merged@assays$RNA@data[VariableFeatures(merged),])
X_scale <- proxyC::rowSds(merged@assays$RNA@data[VariableFeatures(merged),])
retval <- irlba::irlba(A = t(merged@assays$RNA@data[VariableFeatures(merged),]), 
...
merged = FindNeighbors(merged, dims=1:2, reduction = 'umap')
merged = FindClusters(merged, resolution = .1)
test = FindMarkers(merged, ident.1 = 30, only.pos = T, min.diff.pct = .5)
test = wilcoxauc(merged)

DimPlot(merged, group.by = 'orig.ident') + NoAxes() + NoLegend()
FeaturePlot(merged, features = 'XIST', order=T,max.cutoff = 'q99') + NoAxes() + scale_color_viridis()
